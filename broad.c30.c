...
void Stark(AtomicLine *line, double *GStark)
{
  ...
  double C4, C, Cm, m_electron = M_ELECTRON/AMU, cStark23, cStark,
         ...
         E_Rydberg,aa_0 = 1.13807,aa_1 = -1.54913,aa_2 = 0.13423,ab_0 = 1.36331,ab_1 = -1.62649,ab_2 = 0.15218,gstark_aa;
  Atom  *atom = line->atom;
  ...
  if (line->cStark < 0.0) {
    ...
    /* ---Modified by Yj Zhu @ PKU to introduce more accurate quadratic
    ...
    if (atom->weight < 25 && atom->weight > 23 && atom->stage[line->i] == 1&& ((line->j == 1 && line->i == 0) || (line->j == 2 && line->i == 0) || (line->j == 4 && line->i == 2) || (line->j == 5 && line->i == 1) || (line->j == 5 && line->i == 2))){
      if ((line->j == 1 && line->i == 0) || (line->j == 2 && line->i == 0)){
        for (k = 0;  k < atmos.Nspace;  k++) {
          gstark_aa = pow(10,(aa_0 + aa_1*log10(atmos.T[k]) + aa_2*(pow(log10(atmos.T[k]),2))) )*atmos.ne[k]/1.0e21 / 1e10;
          GStark[k] = 30 * 2 * PI * CLIGHT * gstark_aa / pow(2798.7/1e10,2);
          /*printf("ne %e  T %e  gstark  %e\n",atmos.ne[k],atmos.T[k],gstark_aa);*/
        }
      }
      else{
        if ((line->j == 4 && line->i == 2) || (line->j == 5 && line->i == 1) || (line->j == 5 && line->i == 2)){
          for (k = 0;  k < atmos.Nspace;  k++) {
            gstark_aa = pow(10,(ab_0 + ab_1*log10(atmos.T[k]) + ab_2*(pow(log10(atmos.T[k]),2))) )*atmos.ne[k]/1.0e21 / 1e10;
            GStark[k] = 30 * 2 * PI * CLIGHT * gstark_aa / pow(2796.3/1e10,2);
          }          
        }
      }
    }
